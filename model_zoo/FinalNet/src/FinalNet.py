...
class FinalNet(BaseModel):
    ...
    #     loss = self.loss_fn(return_dict["y_pred"], y_true, reduction='mean')
    #     # len = y_true.shape[0]
    #     # data = pd.DataFrame(index=range(len),columns=['y_pred','y1','y2','y1_sig','y2_sig'])

    #     # data['y_pred'] = return_dict['y_pred'].detach().cpu().numpy()
    #     # data['y1'] = return_dict['y1'].detach().cpu().numpy()
    #     # data['y2'] = return_dict['y2'].detach().cpu().numpy()

    #     if self.block_type == "2B":
    #         y1 = self.output_activation(return_dict["y1"])
    #         y2 = self.output_activation(return_dict["y2"])
    #         # data['y1_sig'] = y1.detach().cpu().numpy()
    #         # data['y2_sig'] = y2.detach().cpu().numpy()
    #         loss1 = self.loss_fn(y1, return_dict["y_pred"].detach(), reduction='mean')
    #         loss2 = self.loss_fn(y2, return_dict["y_pred"].detach(), reduction='mean')
    #         loss = loss + loss1 + loss2
    #     # data.to_csv('FinalNet_Nocl.csv',index=False)
    #     return loss
