...
class xDeepFM(BaseModel):
    def __init__(self, 
        ...
        # 确保 optimizer 和 loss 在 kwargs 中
        if "optimizer" not in kwargs or "loss" not in kwargs:
            raise ValueError("Optimizer 和 loss 必须在 kwargs 中指定")
    ...
    def compile(self, optimizer, loss, learning_rate):
        print(f"Compiling with optimizer: {optimizer}, loss: {loss}")
        if optimizer == "adam":
        ...
        print(f"Optimizer initialized: {self.optimizer}")
